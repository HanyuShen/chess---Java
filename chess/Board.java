...
public class Board {
	...
	//This method requires your input
	public static boolean movePiece(int iIn, int jIn, int iOut, int jOut, Piece p){
		boolean a = Board.hasPiece(iOut,jOut);
		if(a){
			if(Board.getPiece(iOut,jOut).getClass().equals(King.class)){
				return false;
			}
			board[iIn][jIn].removePiece();
			Board.setPiece(iOut,jOut,p);
			return true;
		}
		board[iIn][jIn].removePiece();
		Board.setPiece(iOut,jOut,p);
